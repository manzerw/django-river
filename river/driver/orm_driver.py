...
class OrmDriver(RiverDriver):
    def get_available_approvals(self, as_user):
        those_with_max_priority = With(
            TransitionApproval.objects.filter(workflow=self.workflow, status=PENDING)
            .values("workflow", "object_id", "transition")
            .annotate(min_priority=Min("priority"))
        )

        workflow_objects = With(
            self.wokflow_object_class.objects.all(), name="workflow_object"
        )

        approvals_with_max_priority = (
            those_with_max_priority.join(
            ...
            .with_cte(those_with_max_priority)
            .annotate(
        ...
        return (
            ...
            .filter(
                transition__source_state=getattr(
                    workflow_objects.col, self.field_name + "_id"
                )
            )
        )
