...
class TransitionApprovalMeta(BaseModel):
    ...
    permissions = models.ManyToManyField(
        app_config.PERMISSION_CLASS,
        verbose_name=_("Permissions"),
        blank=True,
    )
    groups = models.ManyToManyField(
        app_config.GROUP_CLASS,
    ...
    def __str__(self):
        return (
            f"Transition: {self.transition_meta}, "
            f"Permissions: {','.join(self.permissions.values_list('name', flat=True))}, "
            f"Groups: {','.join(self.groups.values_list('name', flat=True))}, "
            f"Order: {self.priority}"
        )


def post_save_model(sender, instance, *args, **kwargs):
    parents = (
        TransitionApprovalMeta.objects.filter(
            workflow=instance.workflow,
            transition_meta__destination_state=instance.transition_meta.source_state,
        )
    ...
    children = (
        TransitionApprovalMeta.objects.filter(
            workflow=instance.workflow,
            transition_meta__source_state=instance.transition_meta.destination_state,
        )
