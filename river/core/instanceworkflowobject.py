...
class InstanceWorkflowObject(object):
    def __init__(self, workflow_object, field_name):
        ...
        self.content_type = app_config.CONTENT_TYPE_CLASS.objects.get_for_model(
    ...
    def initialize_approvals(self):
        if not self.initialized:
            if (
                self.workflow
                and self.workflow.transition_approvals.filter(
                    workflow_object=self.workflow_object
                ).count()
                == 0
            ):
                transition_meta_list = self.workflow.transition_metas.filter(
                    source_state=self.workflow.initial_state
                )
                iteration = 0
                ...
                while transition_meta_list:
                    for transition_meta in transition_meta_list:
                        ...
                        for (
                        ...
                        ) in transition_meta.transition_approval_meta.all():
                            ...
                            transition_approval.permissions.add(
                                *transition_approval_meta.permissions.all()
                            )
                            transition_approval.groups.add(
                                *transition_approval_meta.groups.all()
                            )
                ...
                LOGGER.debug(
                    "Transition approvals are initialized "
                    f"for the workflow object {self.workflow_object}"
                )
    ...
    def on_final_state(self):
        return (
            self.class_workflow.final_states.filter(pk=self.get_state().pk).count() > 0
        )
    ...
    def recent_approval(self):
        try:
            return (
                getattr(self.workflow_object, self.field_name + "_transition_approvals")
    ...
    def jump_to(self, state):
        ...
        try:
            recent_iteration = (
                self.recent_approval.transition.iteration if self.recent_approval else 0
            )
            jumped_transition = (
                getattr(self.workflow_object, self.field_name + "_transitions")
                .filter(
    ...
    def get_available_states(self, as_user=None):
        all_destination_state_ids = self.get_available_approvals(
            as_user=as_user
        ).values_list("transition__destination_state", flat=True)
    ...
    def get_available_approvals(self, as_user=None, destination_state=None):
        qs = self.class_workflow.get_available_approvals(
            as_user,
        ).filter(object_id=self.workflow_object.pk)
    ...
    def approve(self, as_user, next_state=None):
        ...
        if number_of_available_approvals == 0:
            ...
            if available_approvals.count() == 0:
                ...
                raise RiverException(
                    ...
                    (
                        f"Invalid state is given({next_state.__str__()}). "
                        f"Valid states is(are) {','.join([ast.__str__() for ast in available_states])}"
                    ),
                )
        ...
        if approval.peers.filter(status=PENDING).count() == 0:
            ...
            LOGGER.debug(
                f"Workflow object {self.workflow_object} is proceeded "
                f"for next transition. Transition: {previous_state} "
                f"-> {self.get_state()}"
            )

        with self._approve_signal(approval), self._transition_signal(
            has_transit, approval
        ), self._on_complete_signal():
            self.workflow_object.save()
    ...
    def cancel_impossible_future(self, approved_approval):
        ...
        while possible_next_states:
            ...
            possible_next_states = set(
                possible_transitions.values_list("destination_state__label", flat=True)
            )
    ...
    def _transition_signal(self, has_transit, approval):
        return TransitionSignal(
            has_transit, self.workflow_object, self.field_name, approval
        )
    ...
    def _check_if_it_cycled(self, done_transition):
        ...
        return (
            qs.filter(status=DONE).count() > 0
            and qs.filter(status=PENDING).count() == 0
        )
    ...
    def _get_transition_images(self, source_states):
        meta_max_iteration = (
            ...
            .values_list("meta")
            .annotate(max_iteration=Max("iteration"))
        )
        ...
        return Transition.objects.filter(
            ...
            & six.moves.reduce(
                lambda agg, q: q | agg,
                [
                    Q(meta__id=meta_id, iteration=max_iteration)
                    for meta_id, max_iteration in meta_max_iteration
                ],
                Q(pk=-1),
            )
        )
