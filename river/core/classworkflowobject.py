...
class ClassWorkflowObject(object):
    def __init__(self, wokflow_object_class, field_name):
        ...
        self.workflow = Workflow.objects.filter(
            field_name=self.field_name, content_type=self._content_type
        ).first()
    ...
    def initial_state(self):
        workflow = Workflow.objects.filter(
            content_type=self._content_type, field_name=self.field_name
        ).first()
        return workflow.initial_state if workflow else None

    @property
    def final_states(self):
        all_states = TransitionMeta.objects.filter(workflow=self.workflow).values_list(
