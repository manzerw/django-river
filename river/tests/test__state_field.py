...
class StateFieldTest(TestCase):
    def test_shouldInjectTheField(self):  # pylint: disable=no-self-use
        assert_that(
            BasicTestModel, has_property("river", is_(instance_of(RiverObject)))
        )
        assert_that(
        ...
        assert_that(
            test_model.river.my_field, has_property("approve", has_property("__call__"))
        )
        assert_that(
            test_model.river.my_field,
