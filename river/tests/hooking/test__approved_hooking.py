...
class ApprovedHooking(BaseHookingTest):
    def test_shouldInvokeCallbackThatIsRegisteredWithInstanceWhenAnApprovingHappens(
        self,
    ):
        ...
        authorization_policies = [
            ...
            .with_priority(0)
            .with_permission(authorized_permission)
            .build(),
            AuthorizationPolicyBuilder()
            .with_priority(1)
            .with_permission(authorized_permission)
    ...
    def test_shouldInvokeCallbackForTheOnlyGivenApproval(self):
        ...
        assert_that(
            TransitionApproval.objects.filter(meta=flow.transitions_approval_metas[0]),
            has_length(2),
        )
        first_approval = TransitionApproval.objects.filter(
            meta=flow.transitions_approval_metas[0], transition__iteration=0
        ).first()
