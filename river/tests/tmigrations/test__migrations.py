...
class MigrationTests(TestCase):
    """
    This is the case to detect missing migration issues
    like https://github.com/javrasya/django-river/issues/30
    """
    ...
    def test_shouldCreateAllMigrations(self):
        for f in os.listdir("river/migrations"):
            if f != "__init__.py" and f != "__pycache__" and not f.endswith(".pyc"):
                open(os.path.join("river/tests/volatile/river/", f), "wb").write(
                    open(os.path.join("river/migrations", f), "rb").read()
                )

        self.migrations_before = list(
            filter(
                lambda f: f.endswith(".py") and f != "__init__.py",
                os.listdir("river/tests/volatile/river/"),
            )
        )
    ...
    def test__shouldNotKeepRecreatingMigrationsWhenNoChange(self):
        ...
        self.migrations_before = list(
            filter(
                lambda f: f.endswith(".py") and f != "__init__.py",
                os.listdir("river/tests/volatile/river_tests/"),
            )
        )
    ...
    def test__shouldMigrateTransitionApprovalStatusToStringInDB(self):
        ...
        with connection.cursor() as cur:
            result = cur.execute(
                "select status from river_transitionapproval "
                f"where object_id={workflow_object.model.pk};"
            ).fetchall()
        ...
        with connection.cursor() as cur:
            schema = cur.execute(
                "PRAGMA table_info('river_transitionapproval');"
            ).fetchall()
            status_col_type = list(
                filter(lambda column: column[1] == "status", schema)
            )[0][2]
    ...
    def test__shouldAssessIterationsForExistingApprovals(self):
        ...
        with connection.cursor() as cur:
            result = cur.execute(
                "select river_transitionapproval.meta_id, iteration "
                "from river_transitionapproval inner join "
                "river_transition rt on river_transitionapproval.transition_id = rt.id "
                f"where river_transitionapproval.object_id={workflow_object.model.pk};"
            ).fetchall()
        ...
        with connection.cursor() as cur:
            result = cur.execute(
                "select meta_id, iteration from river_transitionapproval "
                f"where object_id={workflow_object.model.pk};"
            ).fetchall()
            assert_that(result, has_length(4))
            assert_that(result, has_item(equal_to((meta_1.pk, 0))))
    ...
    def test__shouldAssessIterationsForExistingApprovalsWhenThereIsCycle(self):
        ...
        authorized_user = UserObjectFactory(
            user_permissions=[authorized_permission1, authorized_permission2]
        )
        ...
        approvals = TransitionApproval.objects.filter(
            workflow=workflow, workflow_object=workflow_object.model
        )
        assert_that(approvals, has_length(5))

        workflow_object.model.river.my_field.approve(
            as_user=authorized_user, next_state=cycle_state_1
        )
        assert_that(workflow_object.model.my_field, equal_to(cycle_state_1))
    ...
    @skipUnless(*MIGRATION_TEST_ENABLED)
    @skip("Migrations are reset")
    def test__shouldAssessIterationsForExistingApprovalsWhenThereIsMoreAdvanceCycle(
        self,
    ):
        out = StringIO()
        sys.stout = out
        ...
        workflow_object.model.river.my_field.approve(
            as_user=authorized_user, next_state=re_opened
        )
    ...
    @skipUnless(*MIGRATION_TEST_ENABLED)
    @skip("Migrations are reset")
    def test__shouldCreateTransitionsAndTransitionMetasOutOfApprovalMetaAndApprovals(
        self,
    ):
        out = StringIO()
        sys.stout = out
        ...
        transition_1 = TransitionMetaFactory.create(
            workflow=workflow,
            source_state=state1,
            destination_state=state2,
        )
        transition_2 = TransitionMetaFactory.create(
            workflow=workflow,
            source_state=state2,
            destination_state=state3,
        )
        transition_3 = TransitionMetaFactory.create(
            workflow=workflow,
            source_state=state2,
            destination_state=state4,
        )
        ...
        with connection.cursor() as cur:
            result = cur.execute(
                "select id, transition_meta_id from river_transitionapprovalmeta;"
            ).fetchall()
            assert_that(result, has_length(4))
            ...
            result = cur.execute(
                "select id, transition_id from river_transitionapproval "
                f"where object_id='{workflow_object.model.pk}';"
            ).fetchall()
            assert_that(result, has_length(4))
            assert_that(
                ...
                has_item(
                    equal_to(
                        (
                            meta_1.transition_approvals.first().pk,
                            transition_1.transitions.first().pk,
                        )
                    )
                ),
            )
            assert_that(
                ...
                has_item(
                    equal_to(
                        (
                            meta_2.transition_approvals.first().pk,
                            transition_2.transitions.first().pk,
                        )
                    )
                ),
            )
            assert_that(
                ...
                has_item(
                    equal_to(
                        (
                            meta_3.transition_approvals.first().pk,
                            transition_2.transitions.first().pk,
                        )
                    )
                ),
            ...
            assert_that(
                ...
                has_item(
                    equal_to(
                        (
                            meta_4.transition_approvals.first().pk,
                            transition_3.transitions.first().pk,
                        )
                    )
                ),
            )
        ...
        with connection.cursor() as cur:
            schema = cur.execute(
                "PRAGMA table_info('river_transitionapprovalmeta');"
            ).fetchall()
    ...
    def test__shouldMigrateObjectIdInHooksByCastingItToString(self):
        ...
        transition_1 = TransitionMetaFactory.create(
            workflow=workflow, source_state=state1, destination_state=state2
        )
        ...
        callback_function = Function.objects.create(
            name=uuid4(), body=callback_method % str(uuid4())
        )
        ...
        with connection.cursor() as cur:
            schema = cur.execute(
                "PRAGMA table_info('river_onapprovedhook');"
            ).fetchall()
            column_type = next(
                iter([column[2] for column in schema if column[1] == "object_id"]),
                None,
            )
            ...
            schema = cur.execute(
                "PRAGMA table_info('river_oncompletehook');"
            ).fetchall()
            column_type = next(
                iter([column[2] for column in schema if column[1] == "object_id"]),
                None,
            ...
            result = cur.execute(
                "select object_id from river_onapprovedhook "
                f"where object_id='{workflow_object.model.pk}';"
            ).fetchall()
            ...
            result = cur.execute(
                "select object_id from river_ontransithook "
                f"where object_id='{workflow_object.model.pk}';"
            ).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflow_object.model.pk))
            ...
            result = cur.execute(
                "select object_id from river_oncompletehook "
                f"where object_id='{workflow_object.model.pk}';"
            ).fetchall()
            assert_that(result, has_length(1))
            assert_that(result[0][0], equal_to(workflow_object.model.pk))
