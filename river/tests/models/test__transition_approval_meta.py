...
class TransitionApprovalMetaModelTest(TestCase):
    def test_shouldNotDeleteApprovedTransitionWhenDeleted(self):
        ...
        TransitionApproval.objects.filter(workflow=flow.workflow).update(
            status=APPROVED
        )
        ...
        assert_that(
            approvals,
            has_item(has_property("meta", flow.transitions_approval_metas[0])),
        )
    ...
    def test_shouldNotDeletePendingTransitionWhenDeleted(self):
        ...
        assert_that(
            TransitionApproval.objects.filter(workflow=flow.workflow), has_length(1)
        )

        flow.transitions_approval_metas[0].delete()

        assert_that(
            TransitionApproval.objects.filter(workflow=flow.workflow), has_length(0)
        )
