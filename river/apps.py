...
class RiverApp(AppConfig):
    ...
    def ready(self):
        for field_name in self._get_all_workflow_fields():
            try:
                workflows = self.get_model("Workflow").objects.filter(
    ...
    def _get_all_workflow_fields(cls):
        ...
        return reduce(
            operator.concat, map(list, workflow_registry.workflows.values()), []
        )
    ...
    def _register_hook_inlines(self, model):  # pylint: disable=no-self-use
        ...
        if registered_admin:
            if OnApprovedHookInline not in registered_admin.inlines:
                registered_admin.inlines = list(
                    set(
                        registered_admin.inlines
                        + [
                            OnApprovedHookInline,
                            OnTransitHookInline,
                            OnCompleteHookInline,
                ...
                registered_admin.readonly_fields = list(
                    set(
                        list(registered_admin.readonly_fields)
                        + list(workflow_registry.get_class_fields(model))
                    )
                )
